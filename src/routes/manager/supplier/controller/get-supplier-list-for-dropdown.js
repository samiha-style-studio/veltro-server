...
const get_supplier_list_for_dropdown = async (request, res) => {
      try {
            ...
            log.info(`Supplier list for dropdown Found: ${data?.length}`);
...
const generate_data_sql = (request) => {
      let query = `SELECT oid as value, name as label  FROM ${TABLE.SUPPLIER} WHERE status = 'Active' ORDER BY created_on ASC`;
      let values = [];
