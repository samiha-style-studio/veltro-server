...
const get_supplier_list = async (request, res) => {
      try {
            ...
            log.error(`An exception occurred while getting supplier information: ${e?.message}`);
            return res.status(500).json({ code: 500, message: "Something Went Wrong! Please try again later!" });
      }
};

const generate_count_sql = (request) => {
      let query = `SELECT COUNT(*) AS total FROM ${TABLE.SUPPLIER} WHERE 1 = 1`;
      let values = [];
...
const generate_data_sql = (request) => {
      let query = `SELECT * FROM ${TABLE.SUPPLIER} WHERE 1 = 1`;
      let values = [];
