...
const get_employee_attendance_list = async (request, res) => {
      try {
            ...
            return res.status(200).json({
                  code: 200,
                  message: "Employee Attendance list Found",
                  total,
                  data,
            });
      } catch (e) {
            log.error(`An exception occurred while getting employee attendance information: ${e?.message}`);
            return res.status(500).json({ code: 500, message: "Something Went Wrong! Please try again later!" });
      }
};

const generate_count_sql = (request) => {
      let query = `SELECT COUNT(*) AS total FROM ${TABLE.ATTENDANCE} a LEFT JOIN ${TABLE.LOGIN} l ON l.email = a.created_by `;
      let values = [];
...
const generate_data_sql = (request) => {
      ...
      if (request.query.month) {
            query += ` AND TO_CHAR(a.attendance_date, 'YYYY-MM') = $${values.length + 1}`;
            values.push(request.query.month);
      }
...
module.exports = get_employee_attendance_list;
