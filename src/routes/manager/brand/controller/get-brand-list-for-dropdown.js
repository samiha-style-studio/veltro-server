...
const get_brand_list_for_dropdown = async (request, res) => {
      try {
            ...
            log.info(`Brand list for dropdown Found: ${data?.length}`);
...
const generate_data_sql = (request) => {
      let query = `SELECT oid as value, name as label  FROM ${TABLE.BRANDS} WHERE status = 'Active' ORDER BY created_on ASC`;
      let values = [];
