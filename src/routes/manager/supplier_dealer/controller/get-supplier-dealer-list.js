...
const get_supplier_dealer_list = async (request, res) => {
      try {
            ...
            log.info(`Supplier/Dealer list Found: ${data?.length} of ${total}`);
            return res.status(200).json({
                  code: 200,
                  message: "Supplier/Dealer list Found",
                  total,
                  data,
            });
      } catch (e) {
            log.error(`An exception occurred while getting supplier/dealer information: ${e?.message}`);
...
module.exports = get_supplier_dealer_list;
