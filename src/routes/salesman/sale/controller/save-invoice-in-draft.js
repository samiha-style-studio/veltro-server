...
const save_invoice_in_draft = async (request, res) => {
      ...
      try {
            ...
            log.error(`An exception occurred while saving invoice in draft : ${e?.message}`);
            return res.status(500).json({ code: 500, message: "Something Went Wrong! Please try again later!" });
      }
      log.info(`Invoice ${payload.email} saved in draft successfully by : ${user_id}`);
      return res.status(200).json({
            code: 200,
            message: "Invoice Saved in Draft Successfully!",
      });
}
...
module.exports = save_invoice_in_draft;